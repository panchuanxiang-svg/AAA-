...
object Decrypter {
            ...
            val key = when {
                ...
                lowerName.endsWith(".enc2") -> {
                    ...
                    try {
                        ...
                        val v4Key = binaryFileInfo?.v4Key?.first
                        if (v4Key == null) {
                            model.endJob("Unable to retrieve V4 key.")
                            return
                        }
                        v4Key
                    } catch (e: Throwable) {
    ...
    suspend fun onOpenFile(model: DecryptModel) {
        ...
        val decFile = encFile?.let {
            FileManager.saveFile(
                encFile.getName().replace(".enc2", "", ignoreCase = true)
    ...
    fun handleFileInput(model: DecryptModel, info: DecryptFileInfo?) {
        if (info != null) {
            val lowerName = info.encFile.getName().lowercase()
            if (!lowerName.endsWith(".enc2") && !lowerName.endsWith(".enc4")) {
