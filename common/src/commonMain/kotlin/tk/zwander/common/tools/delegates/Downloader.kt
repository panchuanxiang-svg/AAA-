...
object VersionFetch {
    ...
    private fun isTestFirmware(version: String): Boolean {
        return TEST_PATTERNS.any {
    ...
    suspend fun hybridGetLatestVersion(
        ...
        if (history.isNotEmpty()) {
            println(history.joinToString("\n") {
                "SW=${it.swVersion} OS=${it.osName}"
            })

            val beta = history.lastOrNull {
                isTestFirmware(it.swVersion) ||
                    it.osName?.contains("Android 99", true) == true ||
                    it.osName?.startsWith("Z(", true) == true
            }

            println("BETA FOUND = ${beta?.swVersion}")

            val target = beta ?: history.last()

            return FetchResult.VersionFetchResult(
                versionCode = target.swVersion,
                androidVersion = target.osName ?: "",
    ...
    fun parseHistoryInfos(
        ...
        return allInfos.map { info ->
            SmartBinaryInfo(
                ...
                sequence = info.firstDataElementDataByTagName("BINARY_SEQUENCE")
                    ?.toIntOrNull() ?: 0,
